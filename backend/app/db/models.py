...
from db.types import PortableUUID
import enum
...
class Video(Base):
    ...
    id = Column(PortableUUID(), primary_key=True, default=uuid.uuid4)
    project_id = Column(PortableUUID(), ForeignKey("projects.id", ondelete="SET NULL"))
    project_asset_id = Column(PortableUUID(), ForeignKey("project_assets.id", ondelete="SET NULL"), unique=True)
    filename = Column(String(255), nullable=False)
    original_filename = Column(String(255), nullable=False)
    file_path = Column(String(500), nullable=False)
    file_size_bytes = Column(BigInteger)
...
class Transcript(Base):
    ...
    id = Column(PortableUUID(), primary_key=True, default=uuid.uuid4)
    video_id = Column(PortableUUID(), ForeignKey("videos.id", ondelete="CASCADE"), nullable=False, unique=True)
...
class Segment(Base):
    ...
    id = Column(PortableUUID(), primary_key=True, default=uuid.uuid4)
    video_id = Column(PortableUUID(), ForeignKey("videos.id", ondelete="CASCADE"), nullable=False)
