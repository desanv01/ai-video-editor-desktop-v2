...
export default makeScene2D("aive-lecture-composition", function* (view) {
  ...
  const sourceVideo = String(plan.runtime?.sourceVideo || plan.video?.source_url || "");
...
function sceneNode(scene: RenderScene, slide: RenderSlide, sourceVideo: string) {
  ...
  const sourceStart = Number(scene.source_start_time || 0);

  return (
    <Rect width={1920} height={1080} fill={"#070812"}>
      {isCameraFull ? (
        cameraVideo(sourceVideo, 960, 540, 1920, 1080, sourceStart)
      ) : isSideBySide ? (
        <>
          {slideCard(slide, 500, 540, 920, 980)}
          {cameraVideo(sourceVideo, 1440, 540, 880, 980, sourceStart)}
        </>
      ) : (
        <>
          {slideCard(slide, 960, 540, 1920, 1080)}
          {!isSlideOnly && cameraVideo(sourceVideo, 1580, 250, 380, 380, sourceStart)}
        </>
      )}
      {caption && (
