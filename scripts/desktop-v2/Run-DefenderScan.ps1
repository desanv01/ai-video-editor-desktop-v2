...
if (-not $status) {
  $payload = [ordered]@{ schemaVersion = 'desktop.av-scan.v1'; status = 'unavailable'; scanPath = $resolvedScan; startedAt = $started.ToUniversalTime().ToString('o'); detail = 'Microsoft Defender cmdlets are unavailable on this validation host.'; detections = @(); zeroDetectionsClaimed = $false }
