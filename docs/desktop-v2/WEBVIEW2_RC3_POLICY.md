...
The registry detector checks the per-user and per-machine EdgeUpdate client key in both 32-bit and 64-bit views. It also recognizes a fixed runtime only through the explicit `WEBVIEW2_BROWSER_EXECUTABLE_FOLDER` path containing `msedgewebview2.exe`. No network request is made by detection.
